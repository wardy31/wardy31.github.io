<template>
    <div>
      ...
      <img :src="require('@/assets/labeled_images/macabacyao/1.jpg')" alt="eduard" class="ward" id="reference"/>
      </div>
  
      <div style="position: relative;" class="margin">
      <canvas id="overlay1"/>
      <!-- <video id="video" width="720" height="560" autoplay="true" muted ></video> -->
      <img :src="require('@/assets/labeled_images/macabacyao/2.jpg')" alt="eduard" class="ward" id="input"/>
      </div>
      <h4>{{age}} -{{gender}}</h4>
  ...
  export default {
    ...
    methods:{
      ...
      async detectFace(){
        ...
        const faceMatcher = new faceapi.FaceMatcher(result1,0.60)
